...
class Game:
    ...
    async def start(self):
        ...
        await self.ctx.channel.send(embed=discord.Embed(
            title=f":confetti_ball:遊戲已結束！:confetti_ball:以下是成績：", description=final_scores,
    ...
    async def begin_round(self):
        ...
        for user in self.players:
            if user != judge:
                async def wait_for_select(player_waiting: Player):
                    ...
                    await msg.edit_origin(content=f"請回到{self.ctx.channel.mention}等待其他玩家選擇完畢", components=[])
                    return None
        ...
        await question_message.edit(embed=discord.Embed(
        ...
        try:
            winner = int((await wait_for_component(
                ...
                messages=question_message, timeout=120)).custom_id)
        except asyncio.TimeoutError:
            # 若裁判離開遊戲則隨機選擇贏家
            ...
            for player in self.players:
                if player != judge:
                    ...
                    if player.first_card < player.second_card:
                        ...
                        player.cards.pop(player.second_card)
                    if player.second_card < player.third_card:
                        player.cards.pop(player.third_card - 2)
                    elif player.first_card < player.third_card < player.second_card:
                        player.cards.pop(player.third_card - 1)
                    else:
...
class YelloCard(CogExtension):
    def __init__(self, bot):
        self.bot = bot
        self.games = data         # 紀錄當前進行中的遊戲 type: typing.Dict[str, Game]
        self.max_players = 10
        self.min_players = 3
    ...
    async def prepare(self, ctx: ComponentContext):
        players = data[str(ctx.channel_id)]["players"]
    ...
    async def start(self, ctx: SlashContext, score: int, packs: str):
        ...
        if str(ctx.channel_id) in data:
            ...
            data[str(ctx.channel_id)] = {}
            players = data[str(ctx.channel_id)]["players"] = [ctx.author]
        ...
        while True:
            try:
                ...
                await join_message.edit(components=[])
                del data[str(ctx.channel_id)]
                await ctx.send("**超時。**")
                gc.collect()
                return
        self.games[str(ctx.channel_id)] = Game(ctx, score, packs, self.packs, players, self.min_players)
